...
void AutoDetailsView::slotRemoveDetail() {
  ...
  if ( pvitem && ( pvitem->type() == ProjectItem::File ) ) {
    ...
    if (fitem && fitem->is_subst) {
      if (!fitem->m_sRelativeToProjName.isEmpty()) {
        // Do replacement of project_path to $(top_srcdir)
      } else {
...
//     KPopupMenu popup( i18n( "File: %1\nFl: %2" ).arg(fitem->uiFileLink).arg(fitem->m_sRelativeToProjName), this );
    ...
    if (r == idViewUIH) {
      ...
      if (!newFileNames.empty()) {
        ...
        m_subclasslist = DomUtil::readPairListEntry(dom, "/kdevautoproject/subclassing", "subclass", "sourcefile", "uifile");
